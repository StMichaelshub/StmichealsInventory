...
/** Credit Stock Return → Debit Inventory, Credit COGS (reverse cost); Credit A/R (reduce receivable) */
export async function postCreditStockReturnEntry(tx, returnedItems = []) {
  const returnValue = roundCurrency(
    returnedItems.reduce((sum, item) => sum + roundCurrency(item.qty * item.price), 0)
  );
  if (returnValue <= 0) return null;

  // Calculate cost of returned items
  const productIds = returnedItems.map((item) => normalizeId(item.productId)).filter(Boolean);
  await ensureProductCosts(productIds);

  const costOfReturns = roundCurrency(
    returnedItems.reduce((sum, item) => {
      const qty = toNumber(item.qty);
      if (qty <= 0) return sum;
      const productId = normalizeId(item.productId);
      const cost = productId ? toNumber(productCostCache.get(productId)) : 0;
      return sum + (qty * cost);
    }, 0)
  );
  ...
  const customerName = tx?.creditCustomerName || tx?.customerName || "Credit customer";
  const lines = [
    // Reduce A/R by return selling value (customer owes less)
    { code: SYS.AR, credit: returnValue, description: `Credit return - reduce receivable for ${customerName}` },
    // Reverse revenue by the return selling value
    { code: SYS.REVENUE, debit: returnValue, description: `Reverse revenue for credit stock return` },
  ];

  if (costOfReturns > 0) {
    // Inventory comes back on hand
    lines.push({ code: SYS.INVENTORY, debit: costOfReturns, description: "Inventory restored from credit return" });
    // Reverse COGS
    lines.push({ code: SYS.COGS, credit: costOfReturns, description: "Reverse COGS for returned credit stock" });
  }

  return createAutoEntry({
    date: new Date(),
    description: `Credit Stock Return - ${customerName} (${returnedItems.length} item${returnedItems.length > 1 ? "s" : ""})`,
    lines,
    referenceType: "CREDIT_RETURN",
    referenceId: tx?._id,
    reference: tx?._id?.toString(),
    location: tx?.location,
  });
...
export async function syncSystemAccountingEntries() {
  ...
  const [transactions, expenses, purchaseOrders] = await Promise.all([
    Transaction.find({ status: { $in: ["completed", "refunded", "credit"] } })
      .select("_id createdAt updatedAt refundedAt refundReason status total tax tenderType tenderPayments location staffName customerName creditCustomerName creditOriginalTotal creditPaidAmount creditPaidAt creditPayments creditReturnedItems items")
      .lean(),
  ...
  const creditReturnTransactions = creditTransactions.filter((transaction) => Array.isArray(transaction.creditReturnedItems) && transaction.creditReturnedItems.length > 0);
  ...
  const operations = [
    ...
    ...creditReturnTransactions.map((transaction) => postCreditStockReturnEntry(transaction, transaction.creditReturnedItems)),
    ...transactions
  ...
  return {
    ...
    creditReturnsSynced: creditReturnTransactions.length,
